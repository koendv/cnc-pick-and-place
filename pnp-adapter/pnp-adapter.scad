...
nema_boss_dia = 15.0;     // dia boss around shaft
...
optocoupler_pcb_x1 = 22.6 - 10.5/2;
...
module nema8_holes() {
        ...
        nema8_screw_position()
        translate([0, 0, -eps2])
        cylinder(d = nema_washer_dia+2*clearance_fit, h = nema_screw_head+eps2);
        ...
        // hole for stepper
        translate([0,0,h2])
        linear_extrude(nema_body_length)
        offset(nema_clearance)
        square(nema_body_width, center=true);
    
        // hole for connector
        conn_x = nema_connector_x;
        conn_y = nema_connector_y;
        conn_z = nema_connector_z+nema_clearance;
        translate([-nema_body_width/2,-conn_y/2,h2+nema_body_length-conn_z])
        linear_extrude(conn_z)
        offset(nema_clearance)
        mirror([1,0,0])
        square([conn_x, conn_y]);
...
module nema_bridging_layer() {
    /* bridging layer for screw caps */
    difference() {    
        translate([0,0,nema_screw_head])
        cylinder(d=d1,h=clearance_fit);
        cylinder(d = nema_boss_dia, h = h1);
    }
}
...
module optocoupler_body() {
    hull() {
        translate([0,0,optocoupler_z+13])//XXX
        linear_extrude(eps1)
        intersection() {
            translate([optocoupler_pcb_x1,-optocoupler_pcb_y/2])
        ...
        linear_extrude(eps1)
        intersection() {
            translate([optocoupler_pcb_x1,-d3])
            square(2*d3);
            circle(d=d3);
        }
    }
...
module optocoupler_holes() {
    ...
    translate([d3/2 - 0.3,0,optocoupler_z])
    rotate([0,90,0]) {       
        translate([optocoupler_screw_spacing/2,0,0])
        rotate([0,0,180/6])        
        cylinder(d = m3_washer_dia+clearance_fit, h = d3);
